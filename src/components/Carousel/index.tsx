...
export default function Carousel({ heroes, activeId }: IProps) {
  ...
  // Armazenar a posição inicial, no eixo x, da interação com o carrossel
  const [startInteractionPosition, setStartInteractionPosition] =
    useState<number>(0);

  // Som de transição
  const transitionAudio = useMemo(() => new Audio("/songs/transtion.mp3"), []);
  ...
  // Alterar o fundo da página de acordo com o herói selecionado
  useEffect(() => {
    ...
    // Remover a classe quando o componente é desmontado
    return () => {
